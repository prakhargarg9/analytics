...
slice(mtcars, 5:n())
mtcars
slice(mtcars,seq(1,32,2)) #Alternate number
...
mtcars %>% group_by(gear) %>% summarise(meanWT = mean(mpg), n=n()) %>% mutate(newMean = meanWT * 2)
...
mtcars
mtcars %>% group_by(gear, cyl, carb) %>% summarise(mean(mpg),max(hp))
